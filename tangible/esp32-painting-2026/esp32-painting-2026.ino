...
static void broadcastSensorFrame(const SensorFrame& frame) {
  String msg = String("{\"moveY\":") + String(frame.moveY, 3) +
               ",\"rotation\":" + String(frame.rotation, 3) +
               ",\"tilt\":" + String(frame.tilt, 3) +
...
void setup() {
  ...
  delay(1000);
  Serial.println();
  Serial.println("ESP32 painting WebSocket streamer");
...
void loop() {
  ...
  if (millis() - lastSend > SENSOR_SEND_INTERVAL_MS) {
    lastSend = millis();
    broadcastSensorFrame(sensorCodeGetFrame());
  }
